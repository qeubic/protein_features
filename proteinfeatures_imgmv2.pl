...
# The FASTA headers in the sequences from the IMG/M vary in the display of the Genome Name. 
# Identifiers may precede the Genome Name in some FASTA sequence headers. 
# This version of the perl script extracts the content for the Genome Name field to include the square brackets ([]).
...
while (<IN>) {
	...
	if (/^Q\#(.*?)\s\-\s(.*?)\t(.*?)\t(.*?)\t(.*?)\t(.*?)\t(.*?)\t(.*)/) { #match line with Query
		...
		$imgids1=~/\>(.*?)\s(.*?)\s(.*?)(\[.*?\])/; # matching of identifiers and genome name with the square brackets
		$geneid=$1;
		$locus_tag=$2;
		$genename=$3;
		$genomename=$4;
